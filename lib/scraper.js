...
async function searchWithSession(platform, cookies, query) {
  ...
  try {
    ...
    switch (platform) {
      case 'netflix': {
        // Use Netflix's internal search API instead of scraping the UI
        await page.goto('https://www.netflix.com/ar/', { waitUntil: 'networkidle2', timeout: 20000 });
        if (page.url().includes('login')) return { results: [], expired: true };

        // Call Netflix's search API directly
        const searchResults = await page.evaluate(async (q) => {
          try {
            const resp = await fetch(`https://www.netflix.com/search/suggest?q=${encodeURIComponent(q)}&isKidsMode=false&lang=en-US&country=AR`, {
              credentials: 'include',
              headers: { 'x-netflix.request.client.user.guid': '' }
            });
            if (!resp.ok) return [];
            ...
            return (data.suggestions || []).map(s => ({
              ...
              id: s.id || s.nfid || '',
              type: s.type || 'movie'
            })).filter(i => i.title).slice(0, 8);
          } catch(e) {
            // Fallback: try the search page DOM
            window.location.href = `https://www.netflix.com/ar/search?q=${encodeURIComponent(q)}`;
            return [];
          }
        }, query);

        if (searchResults.length === 0) {
          // Fallback to DOM scraping with updated selectors
          ...
          const items = await page.evaluate(() => {
            ...
            const selectors = [
              ...
              '[data-list-context="search"] a',
              '.slider-item a',
              '.jawbone-title-card a',
              'a[href*="/title/"]'
            ];
            for (const sel of selectors) {
              ...
              if (els.length > 0) {
                return Array.from(els).slice(0, 8).map(el => {
                  ...
                  return { title: img?.alt || '', id, link: id ? `https://www.netflix.com/ar/title/${id}` : href };
                }).filter(i => i.title);
              }
            }
            return [];
          ...
          searchResults.forEach(item => results.push({
            title: item.title,
            platforms: ['netflix'],
            url: `https://www.netflix.com/ar/title/${item.id}`,
            confidence: 'high',
            extra_cost: false
          }));
        }
